        ...
        <div className='h-[100vh] font-bold flex justify-center items-center'>
            ...
            {repos && repos.map((repo) => (
                ...
                <a href={repo.node.url} target="_blank" key={repo.node.id} style={{flexBasis:"33.333%"}} className="flex flex-col max-w-sm p-6 m-2 h-48 border rounded-lg shadow bg-black border-gray-700 hover:bg-gray-700">
                    ...
                    <p className="font-normal text-gray-400 w-full max-h-20 overflow-hidden overflow-ellipsis overflow-y-clip text-left">{repo.node.description}</p>
                    <div className="flex items-end flex-grow font-medium text-gray-400 w-80"><div><div style={{backgroundColor: `${repo.node.languages.edges[0].node.color}`}} className="inline-block mr-2 w-3 h-3 rounded-full"></div>{repo.node.languages.edges[0].node.name}</div></div>
                </a>
            ))}
