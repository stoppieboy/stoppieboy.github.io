    ...
    return (
        <div className='h-[100vh] font-bold flex justify-center items-center'>
            ...
            {repos && repos.map((repo) => (
                ...
                <a href={repo.node.url} target="_blank" key={repo.node.id} style={{flexBasis:"33.333%"}} className="flex flex-col max-w-sm p-6 m-2 h-48 bg-white border border-gray-200 rounded-lg shadow hover:bg-gray-100 dark:bg-black dark:border-gray-700 dark:hover:bg-gray-700">
                    ...
                    <div className="flex items-end flex-grow font-medium text-gray-700 dark:text-gray-400 w-80"><div><div style={{backgroundColor: `${repo.node.languages.edges[0].node.color}`}} className="inline-block mr-2 w-3 h-3 rounded-full"></div>{repo.node.languages.edges[0].node.name}</div></div>
                </a>
            ))}
